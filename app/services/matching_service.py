    ...
    def match_candidates(self, job_description: JobDescription, 
        ...
        try:
            ...
            for i, candidate in enumerate(candidates):
                ...
                logger.info(f"Candidate {candidate.display_name}: score = {candidate.score:.4f}")
            ...
            if ranked_candidates:
                max_score = max(c.score for c in ranked_candidates)
                min_score = min(c.score for c in ranked_candidates)
                avg_score = sum(c.score for c in ranked_candidates) / len(ranked_candidates)
                logger.info(f"Score statistics: max={max_score:.4f}, min={min_score:.4f}, avg={avg_score:.4f}, threshold={self.similarity_threshold}")
            ...
            logger.info(f"Matched {len(filtered_candidates)} candidates above threshold {self.similarity_threshold}")
            if not filtered_candidates and ranked_candidates:
                logger.warning(f"No candidates above threshold. Best score was {ranked_candidates[0].score:.4f}")
