...
import docx2txt
import PyPDF2
# import textract  # Commented out due to installation issues
...
class PDFResumeParser(BaseResumeParser):
    ...
    def _extract_text_from_pdf(self, file_path: Path) -> str:
        ...
        try:
            ...
            # textract fallback is not available, raising the original error
            logger.error(f"PDF parsing failed for {file_path}: {e}")
            raise ResumeParsingError(f"Failed to extract text from PDF: {e}")
...
class DocResumeParser(BaseResumeParser):
    ...
    def parse(self, file_path: Path) -> Candidate:
        try:
            ...
            # DOC files require textract which is not available
            # For now, we'll raise an error suggesting to convert to DOCX
            raise ResumeParsingError(
                f"DOC file format is not currently supported. "
                f"Please convert {file_path.name} to DOCX format and try again."
            )
